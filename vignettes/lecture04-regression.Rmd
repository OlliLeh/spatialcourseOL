...
We merge the 2016 employment data with the 2022 spatial dataset using a right join.
...
- tp_tyopy: total number of jobs in 2022
- tp_m22_16: absolute employment change
- tp_m22_16p: percentage change
...
```{r}
t1<-subset(p22_data, kasvu_voima>100); sum(t1$tp_tyopy)/sum(p22_data$tp_tyopy) 
t2<-subset(p22_data, kasvu_kohtal>50); sum(t2$tp_tyopy)/sum(p22_data$tp_tyopy) 
t3<-subset(p22_data, kasvu_heikko>10); sum(t3$tp_tyopy)/sum(p22_data$tp_tyopy) 
t4<-subset(p22_data, kasvu>0); sum(t4$tp_tyopy)/sum(p22_data$tp_tyopy) 
```
